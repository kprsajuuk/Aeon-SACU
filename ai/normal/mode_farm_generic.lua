...
function GetDesire()	
	...
	then
		...
		then
			...
			then
				bot:ActionImmediate_Chat("当前使用的锦囊包即将停止维护, 感谢支持!",true);
			elseif J.Role.IsPastKey()
			then
				bot:ActionImmediate_Chat("当前使用的锦囊包已经停止维护, 感谢你以往的支持!",true);			
			end		
		end	
	...
	if GetGameMode() ~= GAMEMODE_MO 
	   ...
	   and ( DotaTime() > 7 * 60 or bot:GetLevel() >= 7 or ( bot:GetAttackRange() < 220 and bot:GetLevel() >= 5 ) )	   
	then
